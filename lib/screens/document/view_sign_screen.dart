...
class _ViewSignPageState extends State<ViewSignPage>
  ...
  Future<void> _loadWatermark() async {
    try {
      final byteData = await rootBundle.load(AppEnv.watermarkAsset);
      if (mounted) {
        setState(() {
  ...
  Future<void> _saveSignature() async {
    ...
    if (currentTab == 1 && _uploadedImage == null) {
      AppSnackbar.error(context, "Please upload a signature image first.");
      return;
    }
    ...
    if (mounted) {
      ...
      AppSnackbar.success(context, "Signature updated");
